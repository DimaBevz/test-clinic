...
function useTimeTableFormController() {
  ...
  const [isDayOpen, setIsDayOpen] = useState(daysOfWeekEng.reduce((a, v) => ({ ...a, [v]: false}), {}) );
  ...
  const changeAcordionState = (sessionIndex: number) => {
    const currentSessions = getValues("sessionTemplates");
    const updatedSessions = currentSessions.map((session, index) => {
      if (index === sessionIndex) {
        return {
  ...
  useEffect(() => {
    if (isGetTimetableTemplateError && getTimetableTempleteError) {
      errorToast((getTimetableTempleteError as ApiError)?.data?.title as string);
    }
